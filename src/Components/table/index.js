...
const data = [
  {
    company: { text: 'Texla Culture', CbgColor: '#c0f4de' },
  ...
  {
    company: { text: 'Pitch Life', CbgColor: '#f1d5cf' },
  ...
  {
    company: { text: 'O Buddy', CbgColor: ' #f4e4c3' },
          ...
          <TableBody>
            {data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => (
              <TableRow key={row.company.text}>
                {columns.map((column) => (
                  <TableCell key={column.id} style={{padding: "0px 10px 0px 0px", textAlign: 'center'}}>
                    {column.id === 'status' ? (
                      <StatusChip bgColor={row.status.bgColor}>{row.status.text}</StatusChip>
                    ) :column.id === 'company'?  (<CompanyName style={{display: "flex", flexDirection: "row",  alignItems: 'center', padding: "10px"}} bgColor={row.company.CbgColor}>
                        <img src={tc} style={{borderRadius: "50%", margin: '10px'}} alt="" />
                        <div >
                        <p style={{margin: '0px' }}>  {row.company.text}</p> 
                        <p style={{fontSize: "14px", fontWeight: "400", color: "#637381", margin: '0px' }}>INV-17048</p>
                        </div>
